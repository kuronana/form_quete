...
<body>
    <form target="" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"])?>" method="post" class="container">
        <div class="row">
            ...
            <input type="text"  id="nom" name="nom" placeholder="<?php if(isset($error)) echo 'Ex : Rias'?>" required>
            <?php if(isset($error['nom1'])) echo $error['nom1']; ?>
            <?php if(isset($error['nom2'])) echo $error['nom2']; ?>
        </div>
        <div class="row">
            <label for="prenom">Prénom :</label>
            <input type="text" minlength="3" maxlength="30" id="prenom" name="prenom" placeholder="<?php if(isset($error)) echo 'Ex : Gremory'?>" required>
            <?php if(isset($error['prenom1'])) echo $error['prenom1']; ?>
            <?php if(isset($error['prenom2'])) echo $error['prenom2']; ?>
        </div>
        <div class="row">
            <label for="mail">Courriel :</label>
            <input type="email" minlength="3" maxlength="30" id="mail" name="mail" placeholder="<?php if(isset($error)) echo 'Ex : HentaiKamon@ero.fr'?>" required>
            <?php if(isset($error['mail1'])) echo $error['mail1']; ?>
            <?php if(isset($error['mail2'])) echo $error['mail2']; ?>
        </div>
        <div class="row">
            <label for="num">Numero de tel. :</label>
            <input type="tel" id="num" name="num"  placeholder="<?php if(isset($error)) echo 'Ex : 0666778899'?>" required>
        ...
        <div class="row">
            ...
            <textarea rows=10 cols=80 id="message" minlength="3" maxlength="300" name="message" placeholder="<?php if(isset($error)) echo 'Ex : Doit-on se crever les yeux après avoir visionné \'boku no pico\' ?'?>" required></textarea>
